...
import { useVagasStore, categorias } from "@/store/vagasStore";

const Index = () => {
  const { vagasSemana, feirao } = useVagasStore();

  const categoriasComQtd = categorias.map((cat) => {
    const total = vagasSemana.vagas
      .filter((v) => v.categoria === cat.nome)
      .reduce((sum, v) => sum + v.qtd, 0);
    return { ...cat, quantidade: total };
  });
      ...
      <section className="gradient-hero py-12 px-4">
        <div className="container mx-auto text-center">
          <motion.h1
            ...
            className="font-heading font-extrabold text-2xl md:text-4xl text-primary-foreground mb-2"
          >
          ...
          <motion.p
            ...
            className="text-primary-foreground/80 text-sm md:text-base mb-2"
          >
            de João Pessoa
          </motion.p>
          <motion.p
            ...
            className="text-primary-foreground/60 text-xs md:text-sm"
          >
      ...
      <section className="px-4 -mt-6">
        <div className="container mx-auto">
          <motion.div
            ...
            className="bg-card rounded-2xl shadow-card p-6 text-center border border-border"
          >
            ...
            <div className="text-5xl md:text-6xl font-heading font-extrabold text-primary">
              <AnimatedCounter target={vagasSemana.totalVagas} />
            </div>
            <p className="text-muted-foreground text-xs mt-2">
              Período: {vagasSemana.periodoInicio} a {vagasSemana.periodoFim}
            </p>
          </motion.div>
        </div>
      </section>
